...
shared-secret `/medmkp/agent/*` endpoints and never touches the database.

## How it works

1. Backend enqueues a `cart_build_job` when a buyer clicks "Build cart".
2. This runner polls `POST /medmkp/agent/claim`, which returns the job plus the
...
4. The runner reports per-line results + the cart URL via
   `POST /medmkp/agent/result`. The drawer polls and shows status.
...
`/medmkp/agent/claim` returns plaintext passwords, gated only by
